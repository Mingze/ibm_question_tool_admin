...
exports.saveQuestion = function(id, question, profileFields, cb) {
	id=false;
	if (id) {
		//console.log("id", id);
		dbQuestions.get(id, { revs_info: false }, function(err, doc) {
...
exports.getQuestions = function(cb) {
	...
	dbQuestions.view(design, view, qparams, function(err, result) {
		...
		var questions = [];
		var count = 0;
		console.log("result in questeion.js");

		result.rows.forEach(function(row) {
			// 			console.log(row.value);
			var userQuestions = row.value[0];
			var profile_fields = row.value[1];
			// var domaine = row.value[1].value[0];

			// console.log(row.value[1].);
			var name =row.value[2];
			var email = row.value[3];
			var timestamp = row.value[4];
			var team = row.value[5];
			if (userQuestions) {
